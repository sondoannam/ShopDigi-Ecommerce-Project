...
const Enquiries = () => {
  ...
  for (let i = 0; i < enqState.length; i++) {
    data1.push({
      ...
      action: (
        <div className="">
          <Link className="fs-3 text-primary" to={`/admin/enquiries/${enqState[i]._id}`}>
            <AiOutlineEye />
          </Link>

          <button className="ms-3 fs-3 text-danger bg-transparent border-0" onClick={() => showModal(enqState[i]._id)}>
            <AiFillDelete />
          </button>
        </div>
      ),
